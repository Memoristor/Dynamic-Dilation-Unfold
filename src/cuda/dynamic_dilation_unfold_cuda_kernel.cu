...
// Atomic add for at::BFloat16 that works for all CUDA architectures
__device__ __forceinline__ void atomic_add(at::BFloat16* address, at::BFloat16 val) {
  ...
  do {
    assumed = old;
    __nv_bfloat16_raw hsum;
    hsum.x = (size_t)address & 2 ? (old >> 16) : (old & 0xffff);
    __nv_bfloat16 tmp =
        __hadd(*reinterpret_cast<__nv_bfloat16*>(&hsum), static_cast<__nv_bfloat16>(val));
    hsum = *reinterpret_cast<__nv_bfloat16_raw*>(&tmp);
    old = (size_t)address & 2 ? (old & 0xffff) | (hsum.x << 16) : (old & 0xffff0000) | hsum.x;
    old = atomicCAS(address_as_ui, assumed, old);
  } while (assumed != old);
#endif
...
// Overload for float
__device__ __forceinline__ void atomic_add(float* address, float val) { atomicAdd(address, val); }

// Overload for double
__device__ __forceinline__ void atomic_add(double* address, double val) { atomicAdd(address, val); }
...
__device__ scalar_t bilinear_interpolate(const scalar_t* input, const int height, const int width,
  ...
  scalar_t v1 = 0;
  if (h_low >= 0 && w_low >= 0) v1 = input[h_low * width + w_low];

  scalar_t v2 = 0;
  if (h_low >= 0 && w_high < width) v2 = input[h_low * width + w_high];

  scalar_t v3 = 0;
  if (h_high < height && w_low >= 0) v3 = input[h_high * width + w_low];

  scalar_t v4 = 0;
  if (h_high < height && w_high < width) v4 = input[h_high * width + w_high];
...
__global__ void dynamic_dilation_unfold_forward_kernel(
    const int n, const scalar_t* input, const scalar_t* dilation_map, const int batch_size,
    const int channels, const int height, const int width, const int height_out,
    const int width_out, const int kernel_h, const int kernel_w, const int stride_h,
    const int stride_w, const int pad_h, const int pad_w, const int dilation_h,
    const int dilation_w, const int groups, const int channels_per_group, scalar_t* output) {
  CUDA_KERNEL_LOOP(index, n) {
    ...
    const int dilation_map_idx = b * groups * height_out * width_out +
                                 group_idx * height_out * width_out + h_out * width_out + w_out;
...
template <typename scalar_t>
__device__ void bilinear_interpolate_gradient(const scalar_t* input, const int height,
                                              const int width, scalar_t h, scalar_t w,
                                              scalar_t& grad_h, scalar_t& grad_w,
...
__global__ void dynamic_dilation_unfold_backward_input_kernel(
    const int n, const scalar_t* grad_output, const scalar_t* dilation_map, const int batch_size,
    const int channels, const int height, const int width, const int height_out,
    const int width_out, const int kernel_h, const int kernel_w, const int stride_h,
    const int stride_w, const int pad_h, const int pad_w, const int dilation_h,
    const int dilation_w, const int groups, const int channels_per_group, scalar_t* grad_input) {
  CUDA_KERNEL_LOOP(index, n) {
    const int w_out = index % width_out;
    const int h_out = (index / width_out) % height_out;
    const int c_out = (index / width_out / height_out) % (channels * kernel_h * kernel_w);
...
__global__ void dynamic_dilation_unfold_backward_dilation_kernel(
    const int n, const scalar_t* grad_output, const scalar_t* input, const scalar_t* dilation_map,
    ...
    const int dilation_w, const int groups, const int channels_per_group,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    scalar_t grad_h = 0;
    scalar_t grad_w = 0;
    bilinear_interpolate_gradient(input_ptr, height, width, h_in, w_in, grad_h, grad_w,
                                  grad_output[index]);
...
torch::Tensor dynamic_dilation_unfold_cuda_forward(torch::Tensor input, torch::Tensor dilation_map,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      "dynamic_dilation_unfold_forward_cuda", ([&] {
        dynamic_dilation_unfold_forward_kernel<scalar_t>
            <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, at::cuda::getCurrentCUDAStream()>>>(
                num_kernels, input.data_ptr<scalar_t>(), dilation_map.data_ptr<scalar_t>(),
                batch_size, channels, height, width, height_out, width_out, kernel_h, kernel_w,
                stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w, groups,
                channels_per_group, output.data_ptr<scalar_t>());
      }));
...
std::vector<torch::Tensor> dynamic_dilation_unfold_cuda_backward(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::ScalarType::Half, at::ScalarType::BFloat16, input.scalar_type(),
      "dynamic_dilation_unfold_backward_cuda", ([&] {
        dynamic_dilation_unfold_backward_input_kernel<scalar_t>
            <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, at::cuda::getCurrentCUDAStream()>>>(
                num_kernels, grad_output.data_ptr<scalar_t>(), dilation_map.data_ptr<scalar_t>(),
                batch_size, channels, height, width, height_out, width_out, kernel_h, kernel_w,
                stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w, groups,
                channels_per_group, grad_input.data_ptr<scalar_t>());

        dynamic_dilation_unfold_backward_dilation_kernel<scalar_t>
            <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, at::cuda::getCurrentCUDAStream()>>>(
                num_kernels, grad_output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(),
                dilation_map.data_ptr<scalar_t>(), batch_size, channels, height, width, height_out,
                width_out, kernel_h, kernel_w, stride_h, stride_w, pad_h, pad_w, dilation_h,
                dilation_w, groups, channels_per_group, grad_dilation_map.data_ptr<scalar_t>());
      }));
